...
class GraphBuilder:
    ...
    docker_client (DockerClient): docker client used to build the graph
    color_scheme (dict): colors used for the graph
    vm_name (str): name of the virtual machine
    exclude (list): name of containers to exclude of the layout
    '''
                ...
                with vm.subgraph(name = 'cluster_{0}'.format(k)) as network:
                    ...
                    for c in v:
                        with network.subgraph(name = 'cluster_{0}'.format(c.image)) as image:
                            ...
                            if c.ports:
                                ...
                                for p in c.ports:
                                    label += '<{0}> {0}|'.format(p)
                                label = label[:-1] + '}'
                            image.node(c.name, label, color = self.color_scheme['dark_text'], fillcolor = self.color_scheme['container'], fontcolor = self.color_scheme['dark_text'])
                        ...
                        if self.has_traefik and c.url is not None:
                            network.node(c.url, c.url, color = self.color_scheme['traefik'], fillcolor = self.color_scheme['traefik'], fontcolor = self.color_scheme['bright_text'])

            for c in running:
                # Add reverse-proxy links
                if self.has_traefik and c.url is not None:
                    vm.edge('{0}:{1}'.format(self.traefik_container, TRAEFIK_DEFAULT_PORT), c.url, arrowhead = "none", color = self.color_scheme['traefik'])
                    vm.edge(c.url, '{0}:{1}'.format(c.name, c.backend_port), color = self.color_scheme['traefik'])

                # Add links between containers
                for l in c.links:
                    vm.edge('{0}:{1}'.format(c.name, c.name), '{0}:{1}'.format(l, l), color = self.color_scheme['link'])

                # Add port mapping
                for expose, host_ports in c.ports.items():
                    for host in host_ports:
                        vm.node(host, host, shape = 'diamond', fillcolor = self.color_scheme['port'], fontcolor = self.color_scheme['bright_text'])
                        vm.edge(host, '{0}:{1}'.format(c.name, expose), color = self.color_scheme['port'])

        # Render PNG
        self.graph.render(os.path.join(BASE_PATH, 'output', '{0}.gv'.format(self.vm_name)))
